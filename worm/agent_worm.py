...
class AssociativeLearningWormAgent(ActiveInferenceWormAgent):
    ...
    def learn_associations(self, experience_history, learning_rate=0.0005):
        """
    ...
    def _update_observation_model(self, noci_obs, smell_obs, qs, learning_rate):
        ...
        # Very conservative A matrix updates with Dirichlet-style learning
        for state in range(self.num_states):
            belief_weight = qs[0][state]
            
            # Only update if belief weight is significant
            if belief_weight > 0.2:  # Higher threshold for updates
                # Update noci A matrix with much smaller learning rate
                self.A_array[0][:, state] += noci_vec * belief_weight * learning_rate * 0.1
                
                # Update smell A matrix with much smaller learning rate
    ...
    def _update_preferences(self, noci_obs, smell_obs, reward, learning_rate):
        ...
        """
        # Extremely gradual preference learning
        if noci_obs == 0:  # noci present
            # If smell was also present, create negative association
            if smell_obs == 0:  # smell present too
                self.C_vector[1][0] -= learning_rate * 0.01  # extremely gradual aversion to smell
                self.C_vector[1][1] += learning_rate * 0.005  # very slight preference for no smell
        ...
        # If we avoided noci in presence of smell, slightly reduce aversion
            ...
            self.C_vector[1][0] += learning_rate * 0.002  # extremely slight positive update
...
class SimpleLearningAgent(AssociativeLearningWormAgent):
    ...
    def learn(self, history, learning_rate=0.0005, pseudocount=0.01):
        """
